...
public class VFDService implements IVFDService {
    ...
    public void initialize() {
        ...
        try {
            ...
            display = VFDDisplayFactory.createDisplay(config.getDisplayType());
            logger.debug("Created display instance: {}", display.getDisplayModel());

            // Attempt to connect to display
            boolean connected = display.connect(config.getPortName(), config.getBaudRate());
    ...
    public boolean attemptReconnect() {
        ...
        try {
            ...
            IVFDDisplay newDisplay = VFDDisplayFactory.createDisplay(config.getDisplayType());
            boolean connected = newDisplay.connect(config.getPortName(), config.getBaudRate());
